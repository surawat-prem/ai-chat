    ...
    def send_message(self, topic, user_id, chat):
        ...
        if encoded_message is not None:
            try:
                ...
                otlp_logger.info("Sent chat response message to Kafka for user_id: %s", user_id)
            except Exception as e:
                otlp_logger.error("Failed to send chat response message to Kafka for user_id: %s, error: %s", user_id, e)